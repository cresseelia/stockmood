...
股票代码映射（AKShare股票列表 + 本地词典）
    ↓
主体判断（一篇文章多家公司时，由规则或LLM判断受益主体）
    ↓
填充 Signal.stock_code / Signal.stock_name
```
...
- 简单情况（精确匹配）用词典直接映射
- 复杂情况（多公司、模糊指代）标记为待LLM判断，不强行映射
- stock_code 仍无法确定的 Signal 进入"待确认池"，在LLM分析阶段处理
...
输入：经实体识别层处理后的 Signal 列表（stock_code 已填充或标记待确认）

处理逻辑：
1. 过滤掉 stock_code 为 None 且无法推断的信号
2. 按 `stock_code` 分组
3. 统计每只股票的信号数量和来源分布
4. 输出候选股票列表，按信号数降序排列

**关于转载去重**：同一事件被多个源转载会导致信号数虚高。初版先按来源类型去重（同一 source 内的重复内容不重复计数），后续迭代加事件主题聚类。
...
**signals**：原始信号记录（含 metadata，不可丢弃）
```sql
...
2. **context_builder**：上下文信息密度、字段选取、格式
3. **Prompt**：LLM的分析质量、输出一致性
4. **技术形态**：信号定义的准确性
5. **aggregator 去重**：初版按来源类型去重，后续加事件主题聚类
6. **校准基准**：用历史真实案例（华工科技等）端到端回放，验证系统输出是否符合预期
